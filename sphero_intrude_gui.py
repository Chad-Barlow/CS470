...
#Inherited class for Obsticles and goals, and launchers.
#Abstract Parent Class
class Field:
	var center # x y pair
	var radius #Number
	var foi #Feild of influence (range)

	def __init__(self,x,y,radius,foi):
		this.radius = radius
		this.foi = foi
		this.center = x,y

	def getDelta(self,x,y): #abstract function, run in the child classes
		#do something 

#This is a positive node that bb will try to go to
class Goal abs Field:

#This is a negative node that the bb will try to avoid
class Obsticle abs field:

# You implement this class
class Controller:
    stop = True # This is set to true when the stop button is pressed
    
    #create the goal ID:0
    #create launcher ID:3
    #create the obsticles off the info from the apriltags !ID:0 (all others)
    #put all fields into a list
    #update currentpos
    ...
    def trackposCallback(self, msg):
        ...
        #call a function that gets the x and y added up from all the fields
	#it will itereate through all the fields and add up the Dx and Dy and use that to see where bb is going. 
